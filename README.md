...
A voice folder can optionally carry a `bank.json` declaring which recording scheme ("bank type") it uses, plus which normally-merged pronunciation distinctions (자음+ㅢ, 자/쟈, ㅐ/ㅔ, ㅚ/ㅙ) it wants to keep separately recordable — the 253-piece scheme above is the default (`"pieces"`); `"full-syllable"` records every reachable Hangul syllable individually (2,904 by default — not the raw 11,172, since syllables like 쟈 always resolve to 자's recording before a lookup happens); `"diphone"` is a ~510-recording middle tier (onset+vowel blocks joined to vowel+coda tails). All three, plus every distinction setting, are fully supported end-to-end in the CLI/desktop app/recorder tool — the recorder's bank-creation panel shows a live recording-count estimate next to each option. See [`Allinone (2)/TTS_README.txt`](Allinone%20%282%29/TTS_README.txt)'s "사운드 뱅크 종류" section for the manifest format and per-bank audio-setting overrides. Android and web only play `"pieces"` banks today — other types are silently hidden from their voice pickers rather than crashing.
