...
A voice folder can optionally carry a `bank.json` raising its recording completeness beyond the base 253 pieces — not a choice between separate algorithms anymore, but a single cascade tried per syllable: an exact whole-syllable recording (`syllable_overrides`) if present, else an onset+vowel block joined to a vowel+coda tail (`dedicated_diphthongs`, ~510 recordings total) if present, else the base pieces. Four more settings (`preserve_consonant_ui`, `distinguish_palatal_glide`, `distinguish_ae_e`, `distinguish_oe_wae`) each keep one normally-merged pronunciation distinction separately recordable, composing freely with the two above and with each other. The recorder's bank-creation panel presents this as one checkbox list with a live recording-count estimate — "pieces"/"diphone"/"full-syllable" are just one-shot preset buttons that set a sensible starting combination, not exclusive choices. See [`Allinone (2)/TTS_README.txt`](Allinone%20%282%29/TTS_README.txt)'s "사운드 뱅크 설정" section for the manifest format and per-bank audio-setting overrides. Android and web only play the base pieces today — a voice with the higher tiers checked is silently hidden from their voice pickers rather than crashing.
